...
class NNEncLayer(object):
    ...
    def __init__(self, X=32, Y=32, NN=32, sigma=0.5, ENC_DIR='./resources/'):
        self.NN = NN
        self.sigma = sigma
...
class PriorBoostLayer(object):
    ...
    def __init__(self, X=32, Y=32, ENC_DIR='./resources/', gamma=0.5, alpha=1.0):
        self.ENC_DIR = ENC_DIR
        self.gamma = gamma
        self.alpha = alpha
...
class NNEncode:
    ...
    def encode_points_mtx_nd(self, pts_nd, axis=1, returnSparse=False, sameBlock=True):
        ...
        expo = -dists ** 2 / (2 * self.sigma ** 2)
        # expo = expo - np.mean(expo,axis=1,keepdims=True)
        wts = expo
    ...
    def decode_1hot_mtx_nd(self, pts_enc_nd, axis=1, returnEncode=False):
        pts_1hot_nd = np.argmax(pts_enc_nd, axis=axis)
