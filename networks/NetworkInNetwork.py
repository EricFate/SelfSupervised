...
class NetworkInNetwork(nn.Module):
    def __init__(self, num_classes, num_inchannels=3, num_stages=3, use_avg_on_conv3=True):
    ...
    def extract_feature(self, x, out_feat_keys=None):
        if out_feat_keys is None:
            out_feat_keys = ['conv' + str(self.num_stages)]
        return flatten(self.forward(x, out_feat_keys))
...
def create_model(**kwargs):
    return NetworkInNetwork(**kwargs)
